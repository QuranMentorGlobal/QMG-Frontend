...
export default function StudentDashboard() {
  ...
  const [stats, setStats] = useState({ completed: 0, upcoming: 0, active: 0 })
  ...
  useEffect(() => {
    async function load() {
      ...
      // 3. Upcoming lessons (next 10)
      const { data: upcomingLessons } = await supabase
        .from('lessons')
        .select(`
          id, scheduled_at, duration_minutes, status, meeting_url,
          bookings (
            courses ( title ),
            teacher_profiles (
      ...
      // 4. Bookings (my teachers)
      const { data: myBookings } = await supabase
        .from('bookings')
        .select(`
          id, status, total_lessons, lessons_completed,
          courses ( title ),
          teacher_profiles (
            hourly_rate,
            profiles ( full_name, avatar_url )
      ...
      setLessons((upcomingLessons as Lesson[]) ?? [])
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-6 text-sm text-white/80">
              <Link href="/platform/student/dashboard" className="text-white font-semibold border-b-2 border-[#B8952A] pb-0.5">Dashboard</Link>
              <Link href="/platform/student/bookings" className="hover:text-white transition-colors">My Bookings</Link>
              <Link href="/platform/teachers" className="hover:text-white transition-colors">Browse Teachers</Link>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div
              key={stat.label}
              className={`rounded-2xl bg-gradient-to-br ${stat.color} p-6 shadow-sm ${stat.border ? 'border border-[#D4C99A]' : ''}`}
            >
              {loading ? (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className={`text-sm font-medium ${stat.text} opacity-75`}>{stat.label}</p>
                    <p className={`text-4xl font-extrabold ${stat.text} mt-1`}>{stat.value}</p>
                  </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-shrink-0 w-14 h-14 rounded-xl bg-[#F5F0E8] border border-[#D4C99A] flex flex-col items-center justify-center">
                        <span className="text-[10px] font-bold text-[#1B5E37]/60 uppercase">{dt.date.split(' ')[0]}</span>
                        <span className="text-xl font-extrabold text-[#0D3D20] leading-none">{dt.date.split(' ')[1]}</span>
                        <span className="text-[10px] text-[#1B5E37]/60">{dt.date.split(' ')[2]}</span>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {dt.isToday && (
                            <span className="text-xs bg-[#1B5E37]/10 text-[#1B5E37] px-2 py-0.5 rounded-full font-medium">Today</span>
                          )}
                          {dt.isSoon && (
                            <span className="text-xs bg-[#B8952A]/15 text-[#B8952A] px-2 py-0.5 rounded-full font-semibold animate-pulse">Starting soon</span>
                          )}
                        </div>
                      </div>
